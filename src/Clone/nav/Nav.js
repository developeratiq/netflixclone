...
function Nav() {
    ...
    const [Show, setShow] = useState(false); // this is for updating nav background color
    const scrollHandler = ()=>{
        if(window.scrollY>10){   // getting mouse cursor value on scrolling to y axis
            setShow(true);
        }else{
   ...
   useEffect(() => {
      window.addEventListener('scroll', scrollHandler);  // adding event handler to the function
   
     return () => {
        window.removeEventListener('scroll' , scrollHandler);  // complementry 
